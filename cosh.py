...
class ShellType(object):
    ...
    @classmethod
    def env_abbr(cls, key, val):
        return cls.abbr_format.format(
            key=key,
            val=cls.quote_if_needed(val),
        )
...
class Posix(ShellType):
    ...
    abbr_format = 'alias {key}={val}'
    alias_format = abbr_format
    path_sep = ':'
    var_armour = '${{{key}}}'
...
class Abbr(EnvItem):
    def render(self, k, shell):
        return shell.env_abbr(k, self.v)
